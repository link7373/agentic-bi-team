...
3. **Build.** Implementation by tool:
   ...
   - **Power BI:** star-schema model; measures in DAX kept thin (logic upstream); use a dedicated date dimension; name measures exactly as the metrics catalog does. When `{{BI_TOOL}}` is Power BI, run `/powerbi` for the implementation — it builds the dashboard as a PBIP project (TMDL model, PBIR report, theme JSON) with a validation gate, rather than leaving you with import instructions. `standards/powerbi-standards.md` carries the mechanics.
