...
**Filter values persist into the files.** A visual filtered to `Company = 'Contoso'`
stores `Contoso` in `visual.json`. Slicer selections and per-series formatting do the
same. Before committing, check you aren't leaking real customer names into git —
`CLAUDE.md §9` applies to report metadata just as much as to exports.
