...
Respect `{{FISCAL_YEAR_START}}` from `CLAUDE.md` — a fiscal calendar that disagrees
with the business is a reconciliation bug waiting to be discovered by an executive.
...
Never commit a real credential here — `CLAUDE.md §9` and the repo `.gitignore` apply.
