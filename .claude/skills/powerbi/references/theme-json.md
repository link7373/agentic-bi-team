...
Fill these from `standards/dashboard-standards.md` — the placeholders are the same
ones, so there is one place to change a colour:

| Standard | Theme property |
|---|---|
| `{{BRAND_COLORS}}` | `dataColors` |
| `{{GREEN}}` / `{{AMBER}}` / `{{RED}}` | `good` / `neutral` / `bad` |
